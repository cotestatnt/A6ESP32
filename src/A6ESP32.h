...
class A6ESP32 {
public:
    A6ESP32(const byte uart);
    ~A6ESP32();

    byte begin(long baudRate, byte rxPin, byte txPin);
    byte blockUntilReady(long baudRate, byte rxPin, byte txPin);
    char setBaudRate(long baudRate, byte rxPin, byte txPin);
    ...
    byte setSMScharset(String charset);
	  byte sendATString(const char *command);
...
private:
    ...
    long detectRate(byte rxPin, byte txPin);
